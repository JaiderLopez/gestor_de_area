...
- [ ] **13. Desarrollo de la Vista Principal (Dashboard de Discos):**
  - [ ] 13.1. Crear el componente principal `DiscosDashboard` para mostrar la lista de discos.
  - [ ] 13.2. Implementar la lógica para consumir la API `/api/discos/` del backend.
  - [ ] 13.3. Diseñar y desarrollar el componente `DiscoCard` para mostrar información resumida de cada disco.
  - [ ] 13.4. Integrar la paginación (si es necesaria) en el dashboard.

- [ ] **14. Implementación de Búsqueda y Filtros:**
  - [ ] 14.1. Desarrollar componentes de UI para la barra de búsqueda (por nombre de disco, descripción, nombre de contenido).
  - [ ] 14.2. Desarrollar componentes de UI para los filtros avanzados (por tipo de disco, rango de fechas de contenido, rango de tamaño total, rango de espacio libre).
  - [ ] 14.3. Integrar la lógica para enviar las peticiones de filtrado a la API del backend.

- [ ] **15. Desarrollo del Formulario de Registro/Edición de Discos:**
  - [ ] 15.1. Crear el componente `DiscoForm` para añadir nuevos discos o editar existentes.
  - [ ] 15.2. Implementar campos para `nombre`, `tipo`, `tamanio_gb`, `descripcion`.
  - [ ] 15.3. **Funcionalidad de Escaneo de Disco:**
    - [ ] 15.3.1. Implementar un campo de entrada para la ruta del directorio a escanear.
    - [ ] 15.3.2. Integrar la llamada a la API `/api/discos/scan/` con la ruta proporcionada.
    - [ ] 15.3.3. Mostrar los resultados del escaneo (lista de contenidos) en el formulario.
    - [ ] 15.3.4. Permitir al usuario ajustar manualmente el `peso_gb` para las carpetas escaneadas.
  - [ ] 15.4. Implementar la lógica para guardar (POST/PUT) el disco y sus contenidos a través de la API.

- [ ] **16. Manejo de Estado y Conexión con la API:**
  - [ ] 16.1. Utilizar Context API (o Redux) para gestionar el estado global de los discos y contenidos.
  - [ ] 16.2. Crear servicios o utilidades para interactuar con los endpoints de la API del backend.

- [ ] **17. Consideraciones de UI/UX:**
  - [ ] 17.1. Asegurar un diseño responsivo para todos los componentes del módulo.
  - [ ] 17.2. Implementar validaciones en el formulario `DiscoForm`.
  - [ ] 17.3. Mostrar mensajes de carga, éxito y error al usuario.
